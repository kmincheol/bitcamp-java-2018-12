...
public class PhotoBoardCommand{
  ...
  public void detail(Response response) throws Exception {
    ...
    response.println(String.format("수업: %s(%s ~ %s)", board.getLesson().getTitle(),
  ...
  public void list(Response response) {
    ...
    for (PhotoBoard board : boards) {
      response.println(String.format("%3d, %-20s, %s, %d, %d", board.getNo(), board.getTitle(),
  ...
  public void update(Response response) throws Exception {
    ...
    try {
      ...
      String input = response.requestString(String.format("제목(%s)?", origin.getTitle()));

...
//    => 프로젝트명을 적기 싫으면 프로젝트를 루트 애플리케이션으로 설정하라.
//
// [웹 애플리케이션 자동 배포하기]
// 1) 이클립스에 톰캣 서버 경로를 설정한다.
//    - Window/Preference메뉴 클릭
//    - Server 노드 선택
...
public class ServerApp implements Servlet {
  ...
  public void destroy() {
    ...
    // 생성된 모든 서블릿 객체는 소멸 될 것이다.
    // 그래서 소멸 되기 전에 사용한 자원을 해제 시키기 위해 톰캣이 이 메서드를 호출한다.
    // => 즉 init()에서 준비한 자원은 이 메서드에서 해제된다.
  }

...
public class Test08 {
  public static void main(String[] args) {
    // Calculator 클래스를 향후 유지보수 하기 쉽도록 별도의 패키지로 분류 한다.
    ...
    switch (op) {
      ...
      // => 메서드를 사용할 수 없다고 나온다.
      // 이유? 메서드에 접근 허용이 같은 패키지만 가능하기 때문이다.
      // 해결책? 해당 클래스로 가서 메서드를 공개하라!
      case "+":
        result = ch08.util.Calculator2.plus(a, b);
        break;
      case "-":
        result = ch08.util.Calculator2.minus(a, b);
        break;
      case "*":
        result = ch08.util.Calculator2.multiple(a, b);
        break;
      case "/":
        result = ch08.util.Calculator2.divide(a, b);
        break;
      default:

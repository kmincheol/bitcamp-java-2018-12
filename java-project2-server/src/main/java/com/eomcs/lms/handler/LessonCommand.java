...
public class LessonCommand {
  ...
  public LessonCommand(LessonDao lessonDao, PhotoBoardDao photoBoardDao, PhotoFileDao photoFileDao,
      TransactionManager txManager) {
  ...
  public void list(Response response) throws Exception {
    ...
    for (Lesson lesson : lessons) {
      response.println(String.format("%3d, %-15s, %10s ~ %10s, %4d", lesson.getNo(),
          lesson.getTitle(), lesson.getStartDate(), lesson.getEndDate(), lesson.getTotalHours()));
    }
  }
  ...
  public void update(Response response) throws Exception {
    ...
    if (temp.getTitle() != null || temp.getContents() != null || temp.getStartDate() != null
        || temp.getEndDate() != null || temp.getTotalHours() > 0 || temp.getDayHours() > 0) {

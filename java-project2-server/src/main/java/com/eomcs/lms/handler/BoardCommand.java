...
public class BoardCommand {
  ...
  public void add(ServletRequest request, ServletResponse response) throws Exception {
    ...
    PrintWriter out = response.getWriter();
    out.println("<html><head><title>게시물 등록</title><meta http-equiv='Refresh' content='1;url=/board/list'</head>");
  ...
  public void detail(ServletRequest request, ServletResponse response) throws Exception {
    ...
    out.println("<table border='1'>");
    out.printf("<tr><th>번호</th><td><input type='text' name='no' value='%d' readonly></input></tr></td>\n", no);
    out.println(String.format(
        "<tr><th>내용</th><td><textarea name='contents' rows='3' cols='50'>%s</textarea></td></tr>",board.getContents()));
    ...
    out.println("</table>");
    out.println(
        "<p><a href='/board/list'>목록</a><br><a href='/board/delete?no=" + board.getNo() + "'>삭제</a><br>"
  ...
  public void update(ServletRequest request, ServletResponse response) throws Exception {
    ...
    PrintWriter out = response.getWriter();
    out.println(
        "<html><head><title>게시물 변경</title><meta http-equiv='Refresh' content='1;url=/board/list'</head>");
  ...
  public void delete(ServletRequest request, ServletResponse response) throws Exception {
    ...
    PrintWriter out = response.getWriter();
    out.println(
        "<html><head><title>게시물 삭제</title><meta http-equiv='Refresh' content='1;url=/board/list'</head>");

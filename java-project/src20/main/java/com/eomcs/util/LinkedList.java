...
public class LinkedList<E> {
  ...
  // ArrayList 처럼 한번에 큰 메모리를 준비할 필요가 없다.
  // => 그러나 값 이외의 다음 노드와 이전 노드의 주소를 담기 위해 추가로 메모리를 사용한다.
  //
  ...
  public <T> T[] toArray(T[] a) {
    ...
    if (a.length >= size()) {
      ...
      // 만약 파라미터로 받은 배열의 크기가 리스트의 항목 크기 보다 작다면 새로 배열을 만든다.
      arr = (T[]) Array.newInstance(a.getClass() // getClass()의 리턴 값은 T가 아니라 T[] 이다.
          .getComponentType(), // getComponentType()의
          this.size());
    }
  ...
  // => 즉 노드가 다루는 데이터의 타입을 E라고 명명하고 코드를 작성한다.
  // => 노드 클래스를 사용하는 시점에 E가 무슨 타입인지 결정될 것이다.

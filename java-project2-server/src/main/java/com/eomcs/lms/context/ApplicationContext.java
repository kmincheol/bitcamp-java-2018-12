...
public class ApplicationContext {
  ...
  private void preparCommand() throws Exception {
      ...
      for (Class<?> i : interfaces) {
        if (i == Command.class) {
          ...
          if(obj != null) { // 제대로 생성했으면 빈컨테이너에 저장한다.
            // 빈컨테이너에 Command 객체를 저장할 때 key값은 name필드 값으로 한다.
  ...
  private Object createInstanace(Class<?> clazz) throws Exception {
